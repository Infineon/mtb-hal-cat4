...
// #define DEBUG_DMA /* define to get DMA debug output */
...
#define    PCI32ADDR_HIGH        0xc0000000U /* address[31:30] */
#define    PCI32ADDR_HIGH_SHIFT    30        /* address[31:30] */

#define    PCI64ADDR_HIGH        0x80000000U /* address[63] */
    ...
    /* PCI bus with big(>1G) physical address, use address extension */
        if ((di->dataoffsetlow == 0) || !(PHYSADDRLO(pa) & PCI32ADDR_HIGH)) {
            ...
            DMA_ERROR(("low: %p, low: %p, ctrl1: %p, ctrl2: %p\n", (void *) PHYSADDRLO(pa),
                    (void *) di->dataoffsetlow, (void *) _CYHAL_BUS_SWAP32(*flags), (void *) _CYHAL_BUS_SWAP32(ctrl2)));
        ...
        if (di->hnddma.dmactrlflags & DMA_CTRL_PEN) {
            dma64dd_t *dd = &ddring[outidx];
            uint32_t dd_parity = 0UL;
            dd_parity ^= (dd)->addrlow;
            dd_parity ^= (dd)->addrhigh;
            dd_parity ^= (dd)->ctrl1;
            dd_parity ^= (dd)->ctrl2;
            if (m2m_parity32(dd_parity)) {
...
//    }
    ...
    uint32 addrlow;
        ...
        BZERO_SM((void *)(uintptr)di->rxd64, ((uint32_t)(di->nrxd * sizeof(dma64dd_t))));
...
//        bufs_in_dma = NRXDACTIVE(di->rxin, di->rxout);
        ...
        // Need more buffer to RX the packet
        CY_ASSERT(resid <= 0);
        if (resid > 0)
        {
    ...
    bool BCMFASTPATH m2m_dma_rxfill(m2m_hnddma_t *di_in)
    {
        ...
        while ( ( i < n ) && ( ( p = PKTGET(di->osh, (di->rxbufsize + extra_offset +  alignment_req - 1), FALSE) ) != NULL ) )
        {
            ...
            if (CTF_ENAB(kcih)) {
                CY_ASSERT((((uint32)PKTDATA(di->osh, p)) & 31) == 0);
                PKTSETCTF(di->osh, p);
            }
#endif /* CTFMAP */
...
//        bool origcb = TRUE;
    ...
    uint32 addrlow;
        ...
        BZERO_SM((void *)(uintptr)di->txd64, ((uint32_t)(di->ntxd * sizeof(dma64dd_t))));
...
bool m2m_dma_txreset_inner(void *osh, void *txregs)
{
    ...
    if (status != D64_XS0_XS_DISABLED) {
        /* cannot call DMA_ERROR in _CYHAL_ETHERNET_LOG_ENABLE because di is not available */
...
//        for (p = p0; p; p = next)
        {
            ...
            len  = (uint32_t)(PKTLEN(di->osh, p0) - sizeof(wwd_buffer_header_t));
#ifdef BCM_DMAPAD
