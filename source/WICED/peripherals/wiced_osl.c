...
void* memalign(uint32_t param0, uint32_t param1)
{
    return NULL;
}
#endif
...
osl_udelay(unsigned usec)
{
    ...
    uint32_t wait_cycles = (_cyhal_clock_cpu_clock_hz + 500000) / 1000000 * usec;
...
osl_pktget(void *osh, uint len, bool send)
{
    ...
    cy_rslt_t result = host_buffer_get(&buffer, send ? WWD_NETWORK_TX : WWD_NETWORK_RX, len, false);
...
osl_set_ext_lpoclk(uint32 cpu_freq)
{
    ...
    _cyhal_system_gci_chipcontrol(CHIPCTRLREG6, EXT_LPO_SEL | INT_LPO_SEL, EXT_LPO_SEL);
...
{
    _cyhal_system_gci_chipcontrol(CHIPCTRLREG6, EXT_LPO_SEL | INT_LPO_SEL, INT_LPO_SEL);
...
/* Review when ethernet support is needed
int
...
{
#define FORMALIZE(var) {(cc) += ((var) & 0x80000000U) ? 1 : 0; (var) &= 0x7fffffff;}
    uint32 r1, r0;
...
__WEAK cy_rslt_t host_buffer_get(wiced_buffer_t * buffer, wwd_buffer_dir_t direction, unsigned short size, bool wait)
{
    ...
    CY_UNUSED_PARAMETER(wait);
    return CY_RSLT_SUCCESS;
}

__WEAK void host_buffer_release(wiced_buffer_t buffer, wwd_buffer_dir_t direction)
{
    CY_UNUSED_PARAMETER(buffer);
    CY_UNUSED_PARAMETER(direction);
}

__WEAK uint8_t* host_buffer_get_current_piece_data_pointer(wiced_buffer_t buffer)
{
    return (uint8_t *) buffer;
}

__WEAK uint16_t host_buffer_get_current_piece_size(wiced_buffer_t buffer)
{
    CY_UNUSED_PARAMETER(buffer);
    return 0;
}

__WEAK cy_rslt_t host_buffer_set_size(wiced_buffer_t buffer, unsigned short size)
{
    CY_UNUSED_PARAMETER(buffer);
    CY_UNUSED_PARAMETER(size);
    return CY_RSLT_SUCCESS;
...
__WEAK cy_rslt_t host_buffer_add_remove_at_front(wiced_buffer_t * buffer, int32_t add_remove_amount)
{
    CY_UNUSED_PARAMETER(buffer);
    CY_UNUSED_PARAMETER(add_remove_amount);
    return CY_RSLT_SUCCESS;
}

__WEAK wiced_buffer_t host_buffer_get_next_piece(wiced_buffer_t buffer)
{
    CY_UNUSED_PARAMETER(buffer);
    return NULL;
}

__WEAK cy_rslt_t host_buffer_set_next_piece(wiced_buffer_t buffer, wiced_buffer_t next_buffer)
{
    CY_UNUSED_PARAMETER(buffer);
