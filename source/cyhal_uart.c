...
static bool _cyhal_uart_arrays_initialized = false;
CY_NOINIT static cyhal_uart_t* _cyhal_uart_config_structs[_CYHAL_UART_NUM_INSTANCES];
...
{
    if (!_cyhal_uart_arrays_initialized)
    {
        for (uint8_t i = 0; i < _CYHAL_UART_NUM_INSTANCES; i++)
        {
            _cyhal_uart_config_structs[i] = NULL;
        }
        _cyhal_uart_arrays_initialized = true;
    }
    ...
    const cyhal_resource_pin_mapping_t *tx_map = _cyhal_utils_get_resource(tx, cyhal_pin_map_uart_tx,
        sizeof(cyhal_pin_map_uart_tx) / sizeof(cyhal_pin_map_uart_tx[0]), &(obj->resource), false);
    const cyhal_resource_pin_mapping_t *cts_map = (NC != cts) ? _cyhal_utils_get_resource(cts, cyhal_pin_map_uart_cts,
        sizeof(cyhal_pin_map_uart_cts) / sizeof(cyhal_pin_map_uart_cts[0]), &(obj->resource), false) : NULL;
    const cyhal_resource_pin_mapping_t *rts_map = (NC != rts) ? _cyhal_utils_get_resource(rts, cyhal_pin_map_uart_rts,
        sizeof(cyhal_pin_map_uart_rts) / sizeof(cyhal_pin_map_uart_rts[0]), &(obj->resource), false) : NULL;
...
{
    CY_ASSERT(NULL != obj);
    CY_ASSERT(_cyhal_uart_arrays_initialized); /* Should not be freeing if we never initialized anything */
...
cy_rslt_t cyhal_uart_set_async_mode(cyhal_uart_t *obj, cyhal_async_mode_t mode, uint8_t dma_priority)
{
    ...
    {
        result = CYHAL_UART_RSLT_ERR_UNSUPPORTED_OPERATION;
    }
    return result;
}
