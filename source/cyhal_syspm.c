...
*
* \addtogroup group_hal_impl_PM Power Management
* \ingroup group_hal_impl
* \{
*
* CAT4 (CYW943907 and CYW954907) devices use SLEEP and DEEPSLEEP.
* The functions cyhal_syspm_set_system_state() and cyhal_syspm_get_system_state() do not
* affect the power usage of the system.
*
...
*
* cyhal_syspm_sleep() will put the system in a low power mode by stopping the
* Application CPU until the next interrupt wakes it up.
*
* cyhal_syspm_deepsleep() will stop some system clocks and put the system into a
* lower power mode and stop the Application CPU until the next interrupt wakes it up.
*
* \} group_hal_impl_PM
********************************************************************************
* \copyright
...
static void _cyhal_syspm_mcu_powersave_init(void)
{
    ...
    */
    ...
    /* Increase time-up to accommodate above change which spreads power-switch up-delays. */
...
static void _cyhal_syspm_wrappers_foreach(void (*callback)(volatile aidmp_t* dmp, void* ptr), void* ptr, uint32_t type_mask, uint32_t backplane_mask)
{
...
static void _cyhal_syspm_slow_clock_init(void)
{
    ...
    uint32_t       alp_period = ((16UL * 1000000UL) / (osl_alp_clock() / 1024UL));
...
#if 0 /* Not currently using - save for possible future use */
...
/* _cyhal_syspm_system_state is the overall system state.
 * CYHAL_SYSPM_SYSTEM_NORMAL or CYHAL_SYSPM_SYSTEM_LOW
 * - Not CY_CFG_PWR_MODE_SLEEP or CY_CFG_PWR_MODE_DEEPSLEEP
 */
...
{
    static uint32_t deep_sleep_initial_ticks;
    cyhal_lptimer_t *lptimer_obj = _cyhal_lptimer_get_ptr();
    static cyhal_lptimer_info_t lptimer_info;
    cyhal_lptimer_get_info(lptimer_obj, &lptimer_info);
    cyhal_clock_t system_timer_obj;
    cyhal_clock_get(&system_timer_obj, &CYHAL_CLOCK_RSC_CPU);
    uint32_t system_timer_clock_frequency = cyhal_clock_get_frequency(&system_timer_obj);
    ...
    else
    {
            ...
            && (_cyhal_timer_copy != NULL))
        {
            ...
            if (mode == CYHAL_SYSPM_BEFORE_TRANSITION)
            {
                ...
                {
                    timerOn = true;
                    /* save lptimer ticks before deep sleep */
                    deep_sleep_initial_ticks = cyhal_lptimer_read(lptimer_obj);
            ...
            else if (mode == CYHAL_SYSPM_AFTER_TRANSITION)
            {
                if (timerOn)
                {
                    /* Determine how long we were in deepsleep and compute HW ticks to set system timer
                     * so we stay on proper cadence.
                     */
                    if ( (lptimer_obj != NULL) && (_cyhal_timer_copy != NULL) )
                    {
                        uint32_t deep_sleep_final_ticks = cyhal_lptimer_read(lptimer_obj);
                        /* find ticks that have passed, allow for wrap-around counter */
                        uint32_t lpticks_in_deep_sleep = (deep_sleep_final_ticks < deep_sleep_initial_ticks)
                                    ? (lptimer_info.max_counter_value - deep_sleep_initial_ticks) + deep_sleep_final_ticks
                                    : deep_sleep_final_ticks - deep_sleep_initial_ticks;
                        uint32_t divisor = (lptimer_info.frequency_hz / _CYHAL_HZ_TO_KHZ_CONVERSION_FACTOR);
                        uint32_t ms_slept = lpticks_in_deep_sleep / divisor;
                        uint32_t hw_ticks_to_add = lpticks_in_deep_sleep - (ms_slept * divisor );
                        /* Determine the HW ticks to set for the system timer so it hits on the next ms as though it never slept. */
                        _cyhal_timer_copy->previous_stop_hw_ticks += (hw_ticks_to_add * system_timer_clock_frequency) / lptimer_info.frequency_hz;
                    }
